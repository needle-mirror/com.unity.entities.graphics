...
namespace Unity.Rendering.Occlusion.Masked
{
    ...
    public unsafe struct RasterizeJob : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            for (int m = 0; m < Meshes.Length; m += 1)
            {
                float2 max = Meshes[m].screenMax.xy;
                float2 min = Meshes[m].screenMin.xy;

                if (math.any(min > clipRect.zw) || math.any(max < clipRect.xy))
                    continue;

                OcclusionMesh mesh = Meshes[m];
                float* vertex_x = (float*) mesh.vertex_x.GetUnsafePtr();
                float* vertex_y = (float*)mesh.vertex_y.GetUnsafePtr();
                float* vertex_w = (float*)mesh.vertex_w.GetUnsafePtr();
                float2* triangle_max = (float2*)mesh.triangle_max.GetUnsafePtr();
                float2* triangle_min = (float2*)mesh.triangle_min.GetUnsafePtr();
                
                int k = 0;
                for (int j = 0; j < mesh.expandedVertexSize; j += 3, ++k)
                {
                    max = triangle_max[k];
                    min = triangle_min[k];
                    ...
                    for (int n = 0; n < 3; ++n)
                    {
                        temp_stack_x[tempStackSize] = vertex_x[j + n];
                        temp_stack_y[tempStackSize] = vertex_y[j + n];
                        temp_stack_w[tempStackSize] = vertex_w[j + n];
                        tempStackSize++;
                    }
        ...
        void RasterizeTriangleBatch(Tile* tiles, v128* ipVtxX, v128* ipVtxY, v128* pVtxX, v128* pVtxY, v128* pVtxZ, uint triMask, ScissorRect scissor)
        {
            ...
            v128 midTileY = X86.Sse2.srai_epi32(IntrinsicUtils._mmw_max_epi32(midPixelY, X86.Sse2.setzero_si128()), BufferGroup.TileHeightShift + BufferGroup.FpBits);
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                slopeTileRemainder[i] = X86.Sse2.sub_epi32(X86.Sse2.slli_epi32(absEdgeX[i], BufferGroup.FpTileHeightShift), IntrinsicUtils._mmw_mullo_epi32(IntrinsicUtils._mmw_abs_epi32(slopeTileDelta[i]), edgeY[i]));
            ...
            v128 bbBottomIdx = X86.Sse2.add_epi32(bbTileMinX, IntrinsicUtils._mmw_mullo_epi32(bbTileMinY, X86.Sse2.set1_epi32(NumTilesX)));
            v128 bbTopIdx = X86.Sse2.add_epi32(bbTileMinX, IntrinsicUtils._mmw_mullo_epi32(X86.Sse2.add_epi32(bbTileMinY, bbTileSizeY), X86.Sse2.set1_epi32(NumTilesX)));
            v128 bbMidIdx = X86.Sse2.add_epi32(bbTileMinX, IntrinsicUtils._mmw_mullo_epi32(midTileY, X86.Sse2.set1_epi32(NumTilesX)));
            ...
            v128* slopeTileRemainder)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                v128 scanlineSlopeRemainder =
                    X86.Sse2.sub_epi32(IntrinsicUtils._mmw_mullo_epi32(triAbsEdgeX, SimdLaneYCoordI),
        ...
        void TraverseScanline(Tile* tiles, int numRight, int numLeft, int leftOffset, int rightOffset, int tileIdx, int rightEvent, int leftEvent, v128* events, v128 zTriMin, v128 zTriMax, v128 iz0, float zx)
        {
            ...
            {
                right[i] = IntrinsicUtils._mmw_max_epi32(X86.Sse2.sub_epi32(X86.Sse2.srai_epi32(events[rightEvent + i], BufferGroup.FpBits), X86.Sse2.set1_epi32(eventOffset)), X86.Sse2.setzero_si128());
            }

            for (int i = 0; i < numLeft; ++i)
            {
                left[i] = IntrinsicUtils._mmw_max_epi32(X86.Sse2.sub_epi32(X86.Sse2.srai_epi32(events[leftEvent - i], BufferGroup.FpBits), X86.Sse2.set1_epi32(eventOffset)), X86.Sse2.setzero_si128());
            }
        ...
        void UpdateTileAccurate(Tile* tiles, int tileIdx, v128 coverage, v128 zTriv)
        {
            ...
            tiles[tileIdx].zMin1 = IntrinsicUtils._mmw_blendv_ps(z1t, z0, d1min);
            tiles[tileIdx].mask = IntrinsicUtils._mmw_blendv_ps(inner, layerMask0, d1min);
        }
